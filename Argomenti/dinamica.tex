...
\section{Filtraggio dei segnali}
...
\section{Impedenze generalizzate}
	Fino ad ora è stato mostrato come è possibile risolvere un'equazione differenziale lineare può essere risolta come equazione algebrica nel dominio delle frequenze (pag. \ref{sec:four:eqdiff}); l'idea attuale è dunque quella di \de{modellare} i \de{sistemi meccanici} \textbf{a parametri concentrati} convertendoli in \de{schemi alle impedenze generalizzate}.
	
	Questo metodo è comodo in quanto evita di impostare le equazioni differenziali per risolvere i problemi legato alla loro risoluzione, determinando così la funzione di trasferimento con metodologie analoghe a quelle usate per i circuiti elettronici.
	...
	In maniera generale ad ogni parametro concentrato corrisponde un'\textbf{impedenza generalizzata} che permette di relazionare, nel dominio della frequenza, la trasformata della \de{variabile di sforzo} (tensione nel caso elettrico) alla rispettiva \de{variabile di portata} (intensità di corrente nel caso elettrico).
	...
	\paragraph{Parametri concentrati nel sistema meccanico} Considerando la \textbf{massa}, la sua equazione differenziale che lega velocità $v$ e forza $f$ è $ma = m\dot v = F$ che nel dominio delle frequenze diventa l'equazione (e relativa impedenza)
	\begin{equation}
